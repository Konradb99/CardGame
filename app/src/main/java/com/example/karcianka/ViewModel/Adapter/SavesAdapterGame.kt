...
class SavesAdapterGame(private val saves: LiveData<List<GameSave>>, private val viewModel: GameViewModel, context: Context): RecyclerView.Adapter<SavesAdapterGame.SavesHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SavesAdapterGame.SavesHolder {
        val view=LayoutInflater.from(parent.context).inflate(R.layout.one_save_row, parent, false)
        return SavesHolder(view)
    }


    override fun onBindViewHolder(holder: SavesAdapterGame.SavesHolder, position: Int) {
        ...
        holder.loadBtn.setOnClickListener(){
            ...
            holder.myView.findNavController().navigate(R.id.action_fragment_main_game_saves_to_fragment_main_game)
        }
    }
